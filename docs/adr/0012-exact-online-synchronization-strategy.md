...
- **Code** (Edm.String): Auto-generated sequential number, unique per division, immutable. **Note**: Exact reuses Code values—when an account is deleted, its Code may be assigned to a newly created account. Therefore, Code is NOT a reliable unique identifier over time.
