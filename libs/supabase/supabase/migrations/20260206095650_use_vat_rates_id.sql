...
-- 6. Drop old vat_code column from catalog_items (no longer needed as FK)
ALTER TABLE catalog_items
DROP COLUMN vat_code;

-- 7. Swap primary key: drop old vat_code PK, add id as new PK
ALTER TABLE vat_rates
...
-- 8. Add unique constraint on vat_code (preserve lookup integrity)
ALTER TABLE vat_rates
ADD CONSTRAINT vat_rates_vat_code_unique UNIQUE (vat_code);

-- 9. Add FK constraint from catalog_items to vat_rates(id)
ALTER TABLE catalog_items
ADD CONSTRAINT catalog_items_vat_rate_id_fkey
FOREIGN KEY (vat_rate_id) REFERENCES vat_rates(id);

-- 10. Move all MATERIAL catalog items to use the VERLEGD vat rate
