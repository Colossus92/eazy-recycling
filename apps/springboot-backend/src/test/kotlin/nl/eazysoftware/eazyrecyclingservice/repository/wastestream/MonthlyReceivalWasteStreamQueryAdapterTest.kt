...
/**
 * Integration test for MonthlyReceivalWasteStreamQueryAdapter using PostgreSQL Testcontainers.
 *
...
@SpringBootTest
@ActiveProfiles("test")
class MonthlyReceivalWasteStreamQueryAdapterTest : BaseIntegrationTest() {

  @Autowired
  private lateinit var queryAdapter: MonthlyReceivalWasteStreamQueryAdapter
  ...
  @Test
  fun `should find waste streams with weight tickets in the specified month and existing declarations`() {
    // Given - a waste stream with weight tickets in November 2025 AND an existing declaration
    val wasteStreamNumber = "087970000001"
    val yearMonth = YearMonth(2025, 11)

    createWasteStream(wasteStreamNumber, "ACTIVE")
    createLmaDeclaration(wasteStreamNumber, "102025") // Previous declaration exists
    createWeightTicket(
  ...
  fun `should exclude waste streams with weight tickets outside the specified month`() {
    ...
    createWasteStream(wasteStreamNumber, "ACTIVE")
    createLmaDeclaration(wasteStreamNumber, "092025") // Has previous declaration
    createWeightTicket(
      carrierPartyId = carrierCompanyId1,
  ...
  @Test
  fun `should exclude waste streams that have NO previous declarations`() {
    // Given - a waste stream with weight tickets but NO existing declaration (first receival case)
    ...
    // NO declaration created - this should be excluded
  ...
  fun `should exclude cancelled weight tickets from aggregation`() {
    ...
    createWasteStream(wasteStreamNumber, "ACTIVE")
    createLmaDeclaration(wasteStreamNumber, "102025") // Has previous declaration
    createWeightTicket(
      carrierPartyId = carrierCompanyId1,
      weightedAt = OffsetDateTime.of(2025, 11, 15, 10, 0, 0, 0, ZoneOffset.UTC),
      status = "COMPLETED",
  ...
  fun `should only include waste streams for processor 08797`() {
    ...
    createLmaDeclaration(wasteStreamOurs, "102025")
    createLmaDeclaration(wasteStreamOthers, "102025")
    ...
    // Then
    assertThat(results).hasSize(1)
    assertThat(results.first().wasteStreamNumber.number).isEqualTo(wasteStreamOurs)
  }
  ...
  fun `should aggregate multiple weight ticket lines for the same waste stream`() {
    ...
    createWasteStream(wasteStreamNumber, "ACTIVE")
    createLmaDeclaration(wasteStreamNumber, "102025")
    createWeightTicket(
      carrierPartyId = carrierCompanyId1,
  ...
  @Test
  fun `should include waste streams with multiple previous declarations`() {
    // Given - a waste stream with multiple historical declarations
    val wasteStreamNumber = "087970000009"
    val yearMonth = YearMonth(2025, 11)

    createWasteStream(wasteStreamNumber, "ACTIVE")
    createLmaDeclaration(wasteStreamNumber, "092025") // First declaration
    createLmaDeclaration(wasteStreamNumber, "102025") // Second declaration
    createWeightTicket(
      carrierPartyId = carrierCompanyId1,
      weightedAt = OffsetDateTime.of(2025, 11, 15, 10, 0, 0, 0, ZoneOffset.UTC),
      lines = listOf(wasteStreamNumber to 1000.0)
  ...
  @Test
  fun `should handle multiple waste streams in the same month`() {
    // Given - multiple waste streams with weight tickets in the same month
    val wasteStream1 = "087970000010"
    val wasteStream2 = "087970000011"
    val yearMonth = YearMonth(2025, 11)

    createWasteStream(wasteStream1, "ACTIVE")
    createWasteStream(wasteStream2, "ACTIVE")
    createLmaDeclaration(wasteStream1, "102025")
    createLmaDeclaration(wasteStream2, "092025")
    ...
    assertThat(results.map { it.wasteStreamNumber.number })
      .containsExactlyInAnyOrder(wasteStream1, wasteStream2)
  }

  @Test
  fun `should only include INVOICED and COMPLETED weight tickets`() {
    // Given - a waste stream with various weight ticket statuses
    val wasteStreamNumber = "087970000012"
    val yearMonth = YearMonth(2025, 11)
    ...
    createWeightTicket(
      ...
      weightedAt = OffsetDateTime.of(2025, 11, 20, 10, 0, 0, 0, ZoneOffset.UTC),
      status = "DRAFT",
      lines = listOf(wasteStreamNumber to 3000.0) // Should be excluded
    )
