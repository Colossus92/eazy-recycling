...
class MaterialControllerIntegrationTest : BaseIntegrationTest() {
  ...
  fun setup() {
    ...
    // Create a test material group for foreign key constraint
  ...
  fun `should successfully create a material`() {
    ...
    securedMockMvc.post(
      ...
      .andExpect(jsonPath("$.materialGroupId").value(testMaterialGroupId!!))
  ...
  fun `should update material`() {
    ...
    val originalMaterial = MaterialDto(
      code = "MAT005",
      name = "Iron",
      materialGroup = materialGroup,
      unitOfMeasure = "KG",
  ...
  fun `should create multiple materials with same material group`() {
    ...
    assertThat(allMaterials.map { it.materialGroup.id }).allMatch { it == testMaterialGroupId }
  }
