...
export const TrucksTab = () => {
    ...
    const columns: Column<Truck>[] = [
        { key: "licensePlate", label: "Kenteken", width: "20", accessor: (item) => item.licensePlate },
        { key: "brand", label: "Merk", width: "20", accessor: (item) => item.brand },
        { key: "model", label: "Beschrijving", width: "80", accessor: (item) => item.model },
    ];

    const data: DataTableProps<Truck> = {
        columns,
        items: read.items,
    };
