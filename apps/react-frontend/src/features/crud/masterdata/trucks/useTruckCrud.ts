import { TruckRequest } from '@/api/client/models';
import { Truck, truckService } from '@/api/services/truckService';
...
export const useTruckCrud = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<Truck | undefined>(
    undefined
  );
  ...
  const createMutation = useMutation({
    mutationFn: (item: TruckRequest) => truckService.create(item),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (item: TruckRequest) => truckService.update(item),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [queryKey] }).then(() => {
        setItemToEdit(undefined);
        setIsFormOpen(false);
  ...
  const update = async (item: TruckRequest): Promise<void> => {
    return new Promise((resolve, reject) => {
  ...
  return {
    ...
    form: {
      ...
      submit: async (item: TruckRequest) => {
        if (itemToEdit) {
          return update(item);
        } else {
          return create(item);
