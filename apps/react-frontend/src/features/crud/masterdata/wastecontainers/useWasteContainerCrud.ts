...
export const useWasteContainerCrud = () => {
  ...
  const [itemToEdit, setItemToEdit] = useState<WasteContainerView | undefined>(
    undefined
  );
  const [itemToDelete, setItemToDelete] = useState<
    WasteContainerView | undefined
  >(undefined);
  ...
  const createMutation = useMutation({
    mutationFn: (item: WasteContainerRequest) => containerService.create(item),
    onSuccess: () => {
  ...
  const create = async (item: WasteContainerRequest): Promise<void> => {
  ...
  const updateMutation = useMutation({
    mutationFn: (item: WasteContainerRequest) =>
      containerService.update(item.id, item),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [queryKey] }).then(() => {
        setItemToEdit(undefined);
        setIsFormOpen(false);
